...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var STORAGE_PACK = "gamedude.vizPack";
  var STORAGE_PRESET_SLUG = "gamedude.vizPresetSlug";
  var LEGACY_STORAGE_VIBE = "gamedude.vibe";
  var PACK_ORDER = ["base", "extra", "image", "minimal", "nonMinimal", "md1", "other"];
  var PACK_LABELS = {
    base: "Base",
    extra: "Extra",
    image: "Image",
  ...
  var ButterchurnController = class {
    constructor(hostEl, controlsEl) {
      ...
      this._loadedImages = /* @__PURE__ */ new Map();
    ...
    _buildPackOptions() {
      ...
      const packIds = [...counts.keys()].sort(
        (a3, b3) => PACK_ORDER.indexOf(a3) - PACK_ORDER.indexOf(b3)
      );
    ...
    _presetsForPack(packId) {
      ...
      return this._catalog.filter((entry) => entry.packs?.includes(packId)).sort((a3, b3) => a3.key.localeCompare(b3.key));
    }
    _buildPresetOptions() {
    ...
    async _applyCurrentSelection() {
      ...
      const slug = this._presetSelect?.value && !this._presetSelect.value.startsWith("__") ? this._presetSelect.value : localStorage.getItem(STORAGE_PRESET_SLUG);
